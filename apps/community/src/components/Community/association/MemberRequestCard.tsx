...
import { useAcceptJoinRequest, useRejectJoinRequest } from '@/api/association';
import { Button, InfoDisplay } from '@repo/ui';
import { ASSOCIATION_RANK_MAP, INSTITUTION_RANK_MAP } from '@repo/common';
...
const MemberRequestCard = ({ request }: MemberRequestCardProps) => {
  const memberInfo = [
    ...
    {
      title: '직급',
      detail: INSTITUTION_RANK_MAP.EN_TO_KR[request.institutionRank],
    },
  ];
        ...
        <Left>
          <div className="top">
            ...
            <label className="rank">
              {ASSOCIATION_RANK_MAP.EN_TO_KR[request.associationRank]}
            </label>
          </div>
