...
import NoticeIcon from '@repo/ui/src/assets/icons/community/Notice.svg';
...
import { useBoardPosts } from '@/hooks/api/usePostLists';
...
const CommunityDetail = ({ boardType }: CommunityDetailProps) => {
  ...
  const { data, error } = useBoardPosts(
    BOARD_MAP.KR_TO_EN[boardType as keyof typeof BOARD_MAP.KR_TO_EN],
    pageable,
  );
  if (error) {
